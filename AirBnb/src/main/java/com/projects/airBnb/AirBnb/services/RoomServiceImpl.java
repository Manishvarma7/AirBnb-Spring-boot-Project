...
public class RoomServiceImpl implements RoomService {
    ...
    public RoomDto createNewRoom(Long hotelId, RoomDto roomDto) {
        ...
        if(hotel.getActive()){ // here as soon as room is created we are checking if the hotel
            // is active and if active then create an inventory for the room for a year from the start date
    ...
    public List<RoomDto> getAllRoomsInHotel(Long hotelId) {
        ...
        return hotel.getRooms() // returns a list
                .stream()
				.map((element) -> modelMapper.map(element, RoomDto.class)) // list should be converted to stream
